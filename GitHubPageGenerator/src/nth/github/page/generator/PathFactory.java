...
import nth.github.page.generator.model.html.element.page.Width;
...
public class PathFactory {
	...
	private static final String GITHUB_IO = ".github.io";
	...
	public static String createRemoteGitHubWebSiteUri(Config config, nth.github.page.generator.model.Page page) {
		StringBuilder path = new StringBuilder();
		path.append(createRemoteGitHubWebSiteUri(config));
		String relativePath = page.getPath();
		relativePath = relativePath.substring(relativePath
				.lastIndexOf(GITHUB_IO) + GITHUB_IO.length());
		relativePath = relativePath.replace("\\", PATH_SEPARATOR);
		path.append(relativePath);
		return path.toString();
	}

	private static String createRemoteGitHubWebSiteUri(Config config) {
		StringBuilder path = new StringBuilder();
		path.append("http://");
		path.append(config.getGitHubUserName());
		path.append(GITHUB_IO);
		return path.toString();
	}
	...
	public static File createLocalGitHubWebSiteRepositoryPath(Config config) {
		StringBuilder path = new StringBuilder();
		path.append(config.getLocalGitRepositoryLocation());
		path.append(PATH_SEPARATOR);
		path.append(config.getGitHubUserName());
		path.append(GITHUB_IO);
