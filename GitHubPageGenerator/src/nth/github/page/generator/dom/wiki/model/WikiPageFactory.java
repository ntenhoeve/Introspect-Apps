...
public class WikiPageFactory {
	...
	public static void populate(Config config, TextDocument textDocument,    WikiNodeContainer wikiNodeContainer,
			TextNode textNode) {
		...
		if (textNode.getClass() == TextChapterLevel1.class) {
			...
			WikiHyperlink wikiHyperlink = new WikiHyperlink(config, textDocument, textHyperLink);
		...
		if (textNode instanceof TextNodeContainer) {
			...
			for (TextNode child : textNodeContainer.getChilderen()) {
				...
				populate(config, textDocument, wikiNodeContainer, child);
			}
