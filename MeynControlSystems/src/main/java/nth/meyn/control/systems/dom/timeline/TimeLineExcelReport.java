...
public class TimeLineExcelReport {
	...
	public DownloadStream createTimeLineOverview() throws IOException {
		...
		for (LocalDate week = newestOrderWeek; week.isAfter(latestOrderWeek); week = week
				.minusDays(7)) {
			position = addWeek(sheet, position, orders, week, now);
		}

		String fileName = createtFileName();
		DownloadStream downloadStream = WorkBookUtil.createDownloadStream(
				fileName, workbook);
	...
	private String createtFileName() {
		...
		DateTimeFormatter formatter = DateTimeFormat
				.forPattern("YYYY-MM-dd-kk-mm-ss");
		fileName.append("MeynControlSystem-Timeline-");
		fileName.append(formatter.print(LocalDateTime.now()));
	...
	private CustomerOrder latestActiveProject(List<CustomerOrder> orders) {
		for (CustomerOrder customerOrder : orders) {
			if (!customerOrder.isCompleted()
					&& customerOrder.getExWorksDate().getTime() > 10) {
	...
	private Point addWeek(XSSFSheet sheet, Point position,
			List<CustomerOrder> customerOrders, LocalDate date, LocalDate now) {

		int weeksFromNow = Weeks.weeksBetween(date, now).getWeeks();
		// weeksFromNow : -3=over 3 weeks, +5=5 weeks late

		position = addWeekHeader(sheet, position, date, now, weeksFromNow);

		position = addOrders(sheet, position, customerOrders, date,
				weeksFromNow);
	...
	private Point addOrders(XSSFSheet sheet, Point position,
			List<CustomerOrder> customerOrders, LocalDate date, int weeksFromNow) {

		// TODO merge same orders (move factory orders to first similar customer
		// order)
		int yearToFind = date.getYear();
		int WeekToFind = date.getWeekOfWeekyear();
		for (CustomerOrder customerOrder : customerOrders) {
			if (customerOrder.getFactoryOrders().size() > 0) {
				LocalDate orderDate = LocalDate.fromDateFields(customerOrder
						.getExWorksDate());
				int orderYear = orderDate.getYear();
				int orderWeek = orderDate.getWeekOfWeekyear();
	...
	private Point addOrder(XSSFSheet sheet, Point position,
			CustomerOrder customerOrder) {

		String shortOrderTitle = createShortCustomerOrderTitle(customerOrder);

		SheetUtil.setCellText(sheet, position, shortOrderTitle,
				MyCellStyle.CUSTOMER_ORDER_TITLE_STYLE);
		position.translate(1, 0);
		SheetUtil.setCellStyle(sheet, position,
				MyCellStyle.CUSTOMER_ORDER_TITLE_STYLE);
		position.translate(1, 0);
		SheetUtil.setCellStyle(sheet, position,
				MyCellStyle.CUSTOMER_ORDER_TITLE_STYLE);
	...
	private Point addFactoryOrder(XSSFSheet sheet, Point position,
			FactoryOrder factoryOrder) {
		int xStartPos = position.x;
		SheetUtil.setCellText(sheet, position, factoryOrder.getName(),
				MyCellStyle.FACTORY_ORDER_NAME_STYLE);
		position.translate(1, 0);
		...
		for (Activity activity : factoryOrder.getActivities()) {
			...
			switch (status) {
			case ASSIGNED:
				SheetUtil.setCellText(sheet, position, activity.getName()
						.substring(0, 1), MyCellStyle.ACTIVITY_ASSIGNED_STYLE);
				SheetUtil.setCellComment(sheet, position, "Assigned to: "
						+ activity.getScheduledFor().getName(), 7, 1);
				break;
			case COMPLETED:
				SheetUtil.setCellText(sheet, position, activity.getName()
						.substring(0, 1), MyCellStyle.ACTIVITY_COMPLETED_STYLE);
				SheetUtil.setCellComment(sheet, position, "Completed by: "
						+ activity.getCompletedBy().toString(), 7, 1);
				break;
			case NOT_APPLICABLE:
				SheetUtil.setCellText(sheet, position, activity.getName()
						.substring(0, 1),
						MyCellStyle.ACTIVITY_NOT_APPLICABLE_STYLE);
				SheetUtil.setCellComment(sheet, position, "Not Applicable", 7,
						1);
				break;
			case UNASSIGNED:
				SheetUtil
						.setCellText(sheet, position, activity.getName()
								.substring(0, 1),
	...
	private Point addWeekHeader(XSSFSheet sheet, Point position,
			LocalDate date, LocalDate now, int weeksFromNow) {
		...
		text.append(" wk ");
		text.append(date.getWeekOfWeekyear());
		text.append(" (");
		...
		SheetUtil.setCellText(sheet, position, text.toString(),
				MyCellStyle.WEEK_HEADER_STYLE);
		SheetUtil.mergeCells(sheet, position, 0, 5);
	...
	private void addStartActivityZoneTextIfNeeded(XSSFSheet sheet,
			Point position, int weeksFromNow) {
		...
		for (ActivityType activityType : ActivityType.values()) {
			if (activityType.getStartInWeeksFromNow() == (weeksFromNow * -1)) {
				SheetUtil
						.setCellText(sheet, position2, activityType.toString(),
								MyCellStyle.ACTIVITY_ZONE_TEXT);
			}
		}
	...
	private LocalDate getDateOfFirstDayInWeek(Date date) {
		...
		int daysToSubstract = dayOfWeek - DateTimeConstants.MONDAY;
