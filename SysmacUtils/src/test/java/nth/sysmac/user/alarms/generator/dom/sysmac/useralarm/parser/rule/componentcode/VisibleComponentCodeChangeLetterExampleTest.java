...
/**
*
* <h3>Derived component code change letter example</h3>
* Note that the component code letters are changed by the derived component {letter}.<br>Note that the position of {letter} in the data type comment does not matter (prefered is at the beginning) and that the letter case is alwars converted to uppercase.
...
public class VisibleComponentCodeChangeLetterExampleTest extends ExampleTest {
	...
	public VisibleComponentCodeChangeLetterExampleTest() {
		setNote("Note that the component code letters are changed by the derived component {letter}.<br>Note that the position of {letter} in the data type comment does not matter (prefered is at the beginning) and that the letter case is alwars converted to uppercase.");
		...
		addExpectedUserAlarm(MAESTRO, variableName + "." + MAESTRO + "." + BRUSH_VAR+"."+MTR_PROT,
				COMPONENT_CODE_PAGE+"Q"+ COMPONENT_CODE_COLUMN+ " " + MAESTRO + " " + BRUSH_MSG+ " "+MOTOR_PROTECTION_WITHOUT_TAG, Priority.MEDIUM, ExampleTest.NO_ACKNOWLEDGE,
				ExampleTest.NO_DETAILS);
		addExpectedUserAlarm(MAESTRO, variableName + "." + MAESTRO + "." + BRUSH_VAR+"."+NOT_RUNNING,
				COMPONENT_CODE_PAGE+"K"+ COMPONENT_CODE_COLUMN+ " " + MAESTRO + " " + BRUSH_MSG+ " "+MOTOR_NOT_RUNNING_WITHOUT_TAG, Priority.MEDIUM, ExampleTest.NO_ACKNOWLEDGE,
				ExampleTest.NO_DETAILS);
		addExpectedUserAlarm(MAESTRO, variableName + "." + MAESTRO + "." + BRUSH_VAR+"."+NOT_STOPPED,
				COMPONENT_CODE_PAGE+"K"+ COMPONENT_CODE_COLUMN+ " " + MAESTRO + " " + BRUSH_MSG+ " "+MOTOR_NOT_STOPPED_WITHOUT_TAG, Priority.MEDIUM, ExampleTest.NO_ACKNOWLEDGE,
				ExampleTest.NO_DETAILS);
	}
	...
	public static void main(String[] args) {
		VisibleComponentCodeChangeLetterExampleTest v = new VisibleComponentCodeChangeLetterExampleTest();
		System.out.println(v.toJavaDoc());
	}

	@Test
	void testExample() {
		ExampleTest exampleTest = new VisibleComponentCodeChangeLetterExampleTest();
