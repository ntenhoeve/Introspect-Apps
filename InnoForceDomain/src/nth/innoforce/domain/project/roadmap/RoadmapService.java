...
public class RoadmapService {
	...
	private final ProjectRepository projectDataAccess;

	public RoadmapService(ProjectRepository projectDataAccess) {
	...
	public List<ProjectConcept> eviscerationRoadmap() {
		// TODO create a roadmap object that contains project concepts and
		// overall expected intake over the years?
		List<Project> projects = projectDataAccess.getAll();// TODO add
															// creteria:
															// projectStage=ProjectConsept
															// && start project
		List<ProjectConcept> projectConcepts = new ArrayList<ProjectConcept>();

		for (Project project : projects) {
			// if (project.getStage()!=null &&
			// project.getStage()==ProjectStage.ProjectConcept ) {
			ProjectConcept projectConcept = (ProjectConcept) CloneUtil.clone(
