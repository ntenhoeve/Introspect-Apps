...
public class DocumentationFiles {
	...
	private static final List<String> NONE_RESOURCE_EXTENSIONS = Arrays.asList(JavaFile.FILE_EXTENSION, ".jar",
			".class", ".idx", ".pack");
	 ...
	 * 
	 * @param rootFolder
	 *            the actual location of the source folder
	 */

	public DocumentationFiles(File rootFolder) {
	 ...
	 * 
	 * @param referenceName
	 *            a (partial) {@link ReferenceName} to find
	...
	public Optional<ReferenceName> findJavaFileResourceName(ReferenceName referenceName) {
		...
		while (iterator.hasNext()) {
			...
			Optional<ReferenceName> result = referenceNames.stream()
					.filter(r -> r.toString().endsWith(referenceNameToFind)).findFirst();
			if (result.isPresent()) {
				return result;
			}
		}
	 ...
	 * 
	 * @param nameToFind
	 *            (case insensitive), e.g. Image.png or image.png
	 * @return search result
	 */
