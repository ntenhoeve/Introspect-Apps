package nth.reflect.app.swdocgen.dom.javadoc;
...
import nth.reflect.app.swdocgen.dom.documentation.DocumentationInfo;
...
public class JavaDocFactory {
	...
	public static Document getAllJavaDoc(DocumentationInfo documentationInfo, DocumentationFiles documentationFiles)
			throws IOException {

		ReferenceName referenceName = new ReferenceName(documentationInfo.getClassName());
		Optional<JavaFile> result = documentationFiles.findJavaFile(referenceName);

		if (!result.isPresent()) {
			throw new RuntimeException("Could not found: " + referenceName.toString() + " in folder:"
	...
	private static void updateImageSources(Document javaDoc, ReferenceFactory referenceFactory) {
		String query = new JSoupQuery().addElement(ElementName.IMG, AttributeName.SRC + "^=" + ReferenceName.PREFIX)
				.toString();
	...
	private static Map<String, Element> findInternalReferencesAndChapterElements(Document javaDoc) {
		...
		String query = new JSoupQuery().addElement(ElementName.H1).addElement(ElementName.H2).addElement(ElementName.H3)
				.addElement(ElementName.A, AttributeName.ID + "^=" + ReferenceName.PREFIX).toString();
	...
	private static void updateInternalReferences(Document javaDoc, ReferenceFactory referenceFactory) {
		...
		String query = new JSoupQuery().addElement(ElementName.A, AttributeName.HREF + "^=" + ReferenceName.PREFIX)
				.toString();
	...
	/**
	 * Gives all H1, H2 and H3 elements and unique ID so that they can be used
	 * as a book mark (internal or external reference)
	 * 
